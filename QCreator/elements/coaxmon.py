...
#Without JJs
    ...
    def get_center(self):
        ...
        if 'mirror' in self.transformations:
            center = mirror_point(point=self.center, ref1=self.transformations['mirror'][0], ref2=self.transformations['mirror'][1])
        return center

class Coaxmon1(DesignElement):
    """
    Coaxmon consists of several parts:
    1) Central part - central circuit
    params: center = center of the circle/qubit, center_radius = radius of the center part=qubit
    ...
    def render(self):
        ...
        if self.JJ_params is not None:
            # self.JJ_coordinates = (self.center[0] + self.R1 * np.cos(self.JJ_params['angle_qubit']),
            #                        self.center[1] + self.R1 * np.sin(self.JJ_params['angle_qubit']))
            self.JJ_coordinates = (self.center[0] + 2.8 + (self.R1+(self.R2-self.R1)/2) * np.cos(self.JJ_params['angle_qubit']),
                                   self.center[1] + 2.8 + (self.R1+(self.R2-self.R1)/2) * np.sin(self.JJ_params['angle_qubit'])) # For pi/2 angled JJ's (added 31.01.2024)
            # TODO change it in a new manner, probably one day
            if self.import_jj:
                JJ, rect = self.import_JJ(third_JJ=self.third_JJ, file_name=self.file_jj, cell_name=self.cell_jj)
            else:
    ...
    def import_JJ(self, third_JJ, file_name, cell_name):
        ...
        connection_shift = 4
        rect = None
        rect = gdspy.Rectangle((self.JJ_coordinates[0] - 20 - self.JJ.contact_pad_a_outer / 2,
                                self.JJ_coordinates[1] + self.JJ.contact_pad_b_outer - 1.5),
                               (self.JJ_coordinates[0] + self.JJ.contact_pad_a_outer / 2,
                                self.JJ_coordinates[1] + connection_shift - self.JJ.contact_pad_b_outer),
                               layer=self.layer_configuration.total_layer)
        rect.rotate(angle, (self.JJ_coordinates[0], self.JJ_coordinates[1]))
    ...
    def connection_to_ground(self, length, width, JJ_pad_connection_shift=False, coil='old'):
        ...
        for point in [self.JJ.rect1, self.JJ.rect2]:
            # orientation = np.arctan2(-(self.center[1] - (point[1] - length)), -(self.center[0] - point[0]))
            if point == self.JJ.rect1: # here it was just (point[0], point[1] - length) 31.01.2024
                connection_shift = self.JJ.rect_size_b
                orientation = np.arctan2(-(self.center[1] - (point[1] - 3.5*length)), -(self.center[0] - point[0]))
                points = [(point[0], point[1] - connection_shift), (point[0], point[1] - 13 / 4 * length-1.8),
                          (self.center[0] + self.R2 * np.cos(orientation),
                           self.center[1] + self.R2 * np.sin(orientation))]
            else:
                connection_shift = self.JJ.rect_size_b
                orientation = np.arctan2(-(self.center[1] - (point[1] - 5/4 * length)), -(self.center[0] - point[0]))
                points = [(point[0], point[1] - connection_shift), (point[0], point[1] - 5 / 4 * length),
                          (self.center[0] + self.R2 * np.cos(orientation),
                           self.center[1] + self.R2 * np.sin(orientation))]
            # if JJ_pad_connection_shift:
            #     connection_shift = self.JJ.rect_size_b
            # else:
            #     connection_shift = 0
            # points = [(point[0], point[1] - connection_shift), (point[0], point[1] - 5/4*length), # here it was just (point[0], point[1] - length) 31.01.2024
            #           (self.center[0] + self.R2 * np.cos(orientation), self.center[1] + self.R2 * np.sin(orientation))]
            # print(points)
            path = gdspy.FlexPath(deepcopy(points), width, offset=0, layer=self.layer_configuration.total_layer)
            result = gdspy.boolean(path, result, 'or', layer=self.layer_configuration.total_layer)
        # orientation = np.arctan2(-(self.center[1] - (self.JJ.rect1[1] - length)), -(self.center[0] - self.JJ.rect1[0]))
        orientation = np.arctan2(-(self.center[1] - (self.JJ.rect1[1] - 3*length)), -(self.center[0] - self.JJ.rect1[0])) #Small rotation of the FL 31.01.2024
        if coil == 'old':
            bug = 5
            coil_shift = 17
            connection = (self.center[0] + (self.R2 - bug + coil_shift) * np.cos(orientation),
