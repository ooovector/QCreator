...
class Shoe_Transmon(DesignElement):
    """
    Shoe-Transmon consists of several parts:
    ...
    Shoes = position and size of shoe caps
    ...
    def __init__(self, name: str, center: Tuple[float, float],
                 width: float, height: float,gap: float,shoes:Dict,
        ...
        self.shoes = shoes # dictionary with keys 1 to 4 (left top, left bottom, right top, right bottom) and shoe parameters(width and height) measured from the edge of the plates
    ...
    def render(self):
        ...
        for key in self.shoes:
            if key ==1:
                P1 = gdspy.boolean(P1, gdspy.Rectangle((self.center[0]-self.gap/2-self.w,self.center[1]+self.h/2),(self.center[0]-self.gap/2-self.w-self.shoes[key][0],self.center[1]+self.h/2-self.shoes[key][1])), 'or')
            if key ==2:
                P1 = gdspy.boolean(P1, gdspy.Rectangle((self.center[0]-self.gap/2-self.w,self.center[1]-self.h/2),(self.center[0]-self.gap/2-self.w-self.shoes[key][0],self.center[1]-self.h/2+self.shoes[key][1])), 'or')
            if key ==3:
                P2 = gdspy.boolean(P2, gdspy.Rectangle((self.center[0]+self.gap/2+self.w,self.center[1]+self.h/2),(self.center[0]+self.gap/2+self.w+self.shoes[key][0],self.center[1]+self.h/2-self.shoes[key][1])), 'or')
            if key ==4:
                P2 = gdspy.boolean(P2, gdspy.Rectangle((self.center[0]+self.gap/2+self.w,self.center[1]-self.h/2),(self.center[0]+self.gap/2+self.w+self.shoes[key][0],self.center[1]-self.h/2+self.shoes[key][1])), 'or')
