...
from .airbridge import AirbridgeOverCPWMISIS
...
class CrossLinesViaAirbridgesMISIS(DesignElement):
    def __init__(self, name: str, position: Tuple[float, float], orientation: float,
                 top_w: float, top_s: float, top_g: float, bot_w: float, bot_s: float, bot_g: float,
                 narrowing_length: float, geometry: AirBridgeGeometry,
                 line_w=None, line_s=None, line_g=None, with_ground=False):
    ...
    def render(self):
        """
            ...
            like pi or 0.
        """
        b_len = self.geometry.sm_pad_distance + 2 * self.geometry.sm_pad_length
        p_wid = self.geometry.sm_pad_length
        p_wid1 = self.geometry.pad_width
        p_offset = self.geometry.pad_offset

        # 1. Draw two Narrowings for airbridge pads, draft future connection points:
        nar_pos_1 = (self.position[0] - b_len/2 - self.nar_len/2 + p_wid + p_offset, self.position[1])
        nar_pos_2 = (self.position[0] + b_len/2 + self.nar_len/2 - p_wid - p_offset, self.position[1])
        top_1_connection = (nar_pos_1[0] - self.nar_len/2, self.position[1])
        top_2_connection = (nar_pos_2[0] + self.nar_len/2, self.position[1])
        bot_1_connection = (self.position[0], self.position[1] + p_wid/2 + p_wid + self.top_s)
        bot_2_connection = (self.position[0], self.position[1] - p_wid/2 - p_wid - self.top_s)
        ...
        # Draw bridges using existing Airbridge element:
        br_pos_center = (self.position[0], self.position[1])
        bridge_center = AirbridgeOverCPWMISIS('bridge_center', br_pos_center,
                                         orientation=np.pi/2,
                                         w=self.bot_w,
                                         s=self.bot_s,
                                         g=self.bot_g,
        ...
        br_pos_up = (self.position[0], self.position[1] + p_wid + self.top_s)
        bridge_up = AirbridgeOverCPWMISIS('bridge_up', br_pos_up,
                                     orientation=np.pi/2,
                                     w=self.bot_w,
                                     s=self.bot_s,
                                     g=self.bot_g,
        ...
        br_pos_down = (self.position[0], self.position[1] - p_wid - self.top_s)
        bridge_down = AirbridgeOverCPWMISIS('bridge_down', br_pos_down,
                                       orientation=np.pi/2,
                                       w=self.bot_w,
                                       s=self.bot_s,
                                       g=self.bot_g,
        ...
        # Мои изменения начались
        if self.line_w is not None:
            nar_pos_3 = (self.position[0], bridge_down.get_terminals()['port1'].position[1] - self.nar_len / 2 + p_wid1 / 2 - p_wid / 2)
            narrowing_3 = Narrowing('narrowing_3', nar_pos_3,
                                    orientation=np.pi / 2,
                                    w1=self.line_w,
                                    s1=self.line_s,
            ...
            nar_pos_4 = (self.position[0], bridge_up.get_terminals()['port2'].position[1] + self.nar_len / 2 - p_wid1 / 2 + p_wid / 2)
            narrowing_4 = Narrowing('narrowing_4', nar_pos_4,
                                    orientation=-np.pi / 2,
                                    w1=self.line_w,
                                    s1=self.line_s,
