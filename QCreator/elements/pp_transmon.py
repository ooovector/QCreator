...
class PP_Transmon_Coupler:
    ...
    def render(self, center, g_w,g_h,g_t = 0):
        ...
        if self.side == "left":
            ...
            if self.sctq > g_t:
                self.connection = (center[0] - g_w / 2 + g_t+0.001, center[1])
            else:
                self.connection = (center[0]-g_w/2-self.t-self.gap-self.gap+self.sctq-g_t+0.001,center[1])
        ...
        if self.side == "top":
            ...
            #rounding safety is 1 nm :)
            self.connection = (center[0]-g_w/2+self.l1+self.l2/2, center[1]+g_h/2+self.gap+self.t+self.gap-self.sctq+g_t-0.001)
